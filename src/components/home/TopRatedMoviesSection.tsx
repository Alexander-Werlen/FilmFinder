...
import {getTopRatedMovies} from "../../services/home"
...
function TopRatedMoviesSection() {
  const { status, data: topRatedMoviesData } = useQuery({queryKey:["getTopRatedMoviesData"], queryFn: getTopRatedMovies})

  return (
    <div className="pt-6">
      <h1 className="text-white text-xl md:text-2xl lg:text-3xl font-extrabold py-3">Top rated movies</h1>
      <Carousel
      opts={{
        skipSnaps: true,
        slidesToScroll: 1,
      ...
      plugins={[
        ...
        <CarouselContent className="-ml-2">
          {
            (status=="success") &&
            topRatedMoviesData.map((media: MediaCardDataType) => (
